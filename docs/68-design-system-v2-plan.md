...
- [x] 컨트롤·기하·라벨·상태·텍스처·프로토콜과 레시피 후보 측정
- [x] 에셋 인덱스를 역할·상태·크기·알파·팔레트·나인슬라이스로 분류
- [x] 실제 JSON UI 텍스처 참조와 에셋 사용처 연결
- [x] 텍스처 생성용 Skill, 구조화 brief와 source-redacted prompt 추가
- [x] 원본 복사 없이 ignored local corpus와 증분 캐시 생성

정적 수집 스냅샷:

- 로컬 예제 창고: 중립 소스 14개, 지원 파일 32,130개, JSON UI 2,367개, 컨트롤 35,963개, PNG 17,138개
- 스키마 검증 및 `design:search`가 가능한 로컬 디자인 레시피 1,496개
- 에셋 인덱스 입력 40,231개에서 UI 텍스처 3,262개와 고유 SHA 1,869개 정제
- 커스텀 텍스처 사용 연결 1,038건 중 control 소유권 확인 1,014건, 바닐라 사용 연결 4,540건
- 실제 나인슬라이스 값 458쌍과 상태 sibling family 17그룹
- 예제 창고 미해결 163건과 에셋 참조 미해결 항목은 삭제하지 않고 로컬 보고서에 유지

여기서 전체란 저장소 자체, 압축·백업, 생성·캐시, symlink와 미지원 확장자를 제외한 지원 파일 전체를 뜻한다. 원본 자료 전체 복제나 Bedrock 런타임 검증을 뜻하지 않는다.
