...
This document summarizes a local high-quality Bedrock JSON UI reference set.

Use it as a pattern guide for building and adapting JSON UI systems.

## Source Handling Rules

* Extract the implementation details relevant to the target project:

  * routing style
  * collection use
  * animation shape
  * panel sizing
  * screen replacement technique
  * progress-bar logic
* Verify texture and model dependencies before reuse.
* Adapt namespaces, controls, variables, textures, and related assets as needed for the target project.
* Keep examples organized by feature, such as `premium_server_form_router`, `premium_loading_screen`, `premium_hud_progress`, or `premium_container_screen`.
...
5. A view binding checks whether a token or prefix exists.
...
The visible title can be rendered separately inside the custom UI when needed.
...
The reference computes a percentage label from `#loading_bar_percentage` using view bindings. Treat this as a clever but fragile trick. If exact decimals are not required, prefer a simpler label or no label.
...
4. Adapt the JSON structure and naming to the target project.
5. Verify texture paths and asset dependencies before use.
6. Keep routing tokens stable where routing depends on them.
