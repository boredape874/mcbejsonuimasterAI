...
This document summarizes the local JSON UI references mirrored under `references/restricted/json-ui-reference-packs/`.

## How To Use

1. Pick the user's target from `docs/55-reference-task-taxonomy.md` or `docs/57-hierarchical-task-router.md`.
2. Open the matching reference folder if it exists locally.
3. Extract the relevant implementation details:
...
4. Adapt namespaces, controls, variables, and texture paths for the target project as needed.
...
Its raw files live under `references/restricted/advanced-ui-set-ui/`.
...
Verify texture paths and dependencies before integrating the pattern into another pack.
...
Treat the reference as an implementation example and adapt its controls and commands to the target project.
