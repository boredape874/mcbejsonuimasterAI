...
export async function renderDisplayList({ canvasMod, displayList, targetRoot, vanillaRoot = null, outputPath = null, textureEngine = null, analyzeControls = true }) {
  ...
  for(const command of commands){
    ...
    }else if(command.type==="runtimeCustomRenderer") diagnostics.push({kind:"RUNTIME_CUSTOM_RENDERER_UNAVAILABLE",impact:"blocking",control:command.id,renderer:command.renderer,pointer:command.pointer??null,message:`Bedrock engine renderer '${command.renderer}' is not available in the offline renderer`});
