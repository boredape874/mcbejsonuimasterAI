...
const BLOCKING_KINDS = /^(unresolved_(control|dimension|state|texture|glyph|binding|collection|animation)|invalid_ui_property|rejected_texture_path|FONT_UNAVAILABLE|GLYPH_RUN_UNAVAILABLE|RUNTIME_CUSTOM_RENDERER_UNAVAILABLE)/i;
...
function inferStage(kind="") { return /binding|collection|expression/.test(kind)?"data":/state|animation/.test(kind)?"state":/dimension|layout|hit|occlusion/.test(kind)?"layout":/texture|glyph|font|renderer/.test(kind)?"render":"resolve"; }
