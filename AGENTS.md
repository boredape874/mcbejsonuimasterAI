...
| "?¥Í±∏Î°??§Ï†ú RP ÎßåÎì§??Ï§? / "production-ready" / "skills Í∏∞Î∞ò?ºÎ°ú ÎßàÍ∞ê" | **Two-stage**: tools for coords, then hand-finish the JSON UI per `docs/46-tools-output-to-handcrafted-ui.md` (3-state buttons, vanilla nineslice, modification-only routing). **MD docs and `references/sample-packs/*` are authoritative; tool `ok=true` is not sufficient ??see `docs/26` and `docs/46` Authority order.** |
